...
typedef struct {
  int		  character;	/* key */
  int		  hinting;	/* key */
  int             gray_render;  /* key */ /* SDL modification 5/13/99 */
  font_t	  *font;
} glyphkey_t;
...
/********************************************************************/
/* imageColorResolve is a replacement for the old fragment:              */
/*                                                                  */
/* This exists as gdImageColorResolve in newer versions of GD. It   */
/* included here for backwards compatability. Same code though.     */
/*                                                                  */
...
static int imageColorResolve(gdImagePtr im, int r, int g, int b)
{
  int c; 
  int ct = -1;
  int op = -1;
...
glyphTest ( void *element, void *key )
{
  ...
  return (a->character == b->character && a->hinting == b->hinting && a->gray_render == b->gray_render);  /* SDL modification 5/13/99 */
  /* return (a->character == b->character && a->hinting == b->hinting); */
}
...
glyphFetch ( char **error, void *key )
{
	...
	a->gray_render = b->gray_render; /* SDL modification 5/13/99 */
...
static void *tweenColorFetch (char **error, void *key)
{
  ...
  if (fg <0) {
    ...
    a->tweencolor = imageColorResolve(im,
...
char *gdttfchar(gdImage *im, int fg, font_t *font,
		...
		char **next)
{
  ...
  static gdCache_head_t *tweenColorCache=NULL; /****** set up tweenColorCache on first call ************/
  ...
  /* SDL modification 5/13/99 */
  if(fg < 0)
    glyphkey.gray_render = FALSE;
  else
    glyphkey.gray_render = TRUE;
...
#ifndef USE_GD_1_2 
	pixel = &im->pixels[y3][x3]; /* row major */
#else	
	pixel = &im->pixels[x3][y3]; /* column major */
#endif
...
/********************************************************************/
/* imageStringTTF -  render a utf8 string onto a gd image           */ 
/*                                                                  */
/* Renamed from gdImageStringTTF to avoid conflicts with newer      */
/* versions of GD.                                                  */

char * imageStringTTF(gdImage *im, int *brect, int fg, char *fontname, double ptsize, double angle, int x, int y, char *string, char wrap)
{
  ...
  char *tmpstr = 0; /* 1.7.2: initialize this variable. */
  
  static gdCache_head_t	*fontCache=NULL; /****** initialize font engine on first call ************/
  ...
  if (font->have_char_map_Sjis) {
#endif
    tmpstr = (unsigned char *)malloc(BUFSIZ);
    if (tmpstr) {
  ...
  while (*next) {	  
    ...
    if(ch == wrap) { /* sdl modification 5/13/99 */
      advance_x = 0;
      advance_y -= (TT_F26Dot6)(font->imetrics.y_ppem * LINESPACE * 64);
      advance_y = (advance_y-32) & -64; /* round to next pixel row */
      next++;
