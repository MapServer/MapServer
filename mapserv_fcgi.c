 ...
 *
 * Revision 1.5  2003/09/24 03:42:12  sdlime
 * Fixed bug 435. There is a new mapObj parameter called MAXSIZE. The default is 2048 as before but it can be changed in the map file now.
 *
 * Revision 1.4  2003/09/19 21:48:46  assefa
 * Modify of the request handling to add support for post requests.
 *
...
static int msReturnURL_FastCGI(mapservObj* msObj, char* url, int mode);
static int msSaveImage_FastCGI(mapObj *map, imageObj *img);
char *processLine(mapservObj* msObj, char* instr, int mode);
static int msRedirect_FastCGI(char *url);
...
void writeError()
{
  ...
  if(!msObj->Map) {
    ...
    FCGI_printf("Content-type: text/html%c%c",10,10);
    FCGI_printf("<HTML>\n");
    FCGI_printf("<HEAD><TITLE>MapServer Message</TITLE></HEAD>\n");
    FCGI_printf("<!-- %s -->\n", msGetVersion());
  ...
  if((ms_error->code == MS_NOTFOUND) && (msObj->Map->web.empty)) {
    msRedirect_FastCGI(msObj->Map->web.empty);
  } else {
    if(msObj->Map->web.error) {      
      msRedirect_FastCGI(msObj->Map->web.error);
    } else {
      errorObj *ms_error = msGetErrorObj();
...
void loadForm()
{
  ...
  for(i=0;i<;i++) { // now process the rest of the form variables
    ...
    */
    ...
    if(strcasecmp(msObj->request->ParamNames[i],"item") == 0) { // search item
      Item = strdup(msObj->request->ParamValues[i]);
      continue;
    }

    if(strcasecmp(msObj->request->ParamNames[i],"value") == 0) { // search expression
      if(!Value)
	Value = strdup(msObj->request->ParamValues[i]);
      else { /* need to append */
	tmpstr = strdup(Value);
	free(Value);
	Value = (char *)malloc(strlen(tmpstr)+strlen(msObj->request->ParamValues[i])+2);
	sprintf(Value, "%s|%s", tmpstr, msObj->request->ParamValues[i]);
	free(tmpstr);
      }
      continue;
    }
...
int main(int argc, char *argv[]) {
	...
	writeError();
     ...
     = loadParams(msObj->request->ParamNames, msObj->request->ParamValues);
    msObj->Map = loadMap();
    ...
    */
    ...
    if ((status = msOWSDispatch(msObj->Map, msObj->request->ParamNames, 
                                ...
                                )) != MS_DONE  ) 
    {
       ...
       */
      msFreeMap(msObj->Map);
      msFreeCharArray(msObj->request->ParamNames, );
      msFreeCharArray(msObj->request->ParamValues, msObj->NumParams);
...
/* -------------------------------------------------------------------- */
      ...
      if(QueryFile) {
          ...
          if (msReturnQuery(msObj, "text/html", &pszReturn) != MS_SUCCESS)
              writeError();
          else
          {
            FCGI_printf("%s", pszReturn);
            msFree( pszReturn );
          }
      } else {
	if(TEMPLATE_TYPE(msObj->Map->web.template) == MS_FILE) { /* if thers's an html template, then use it */
            ...
            FCGI_printf("Content-type: text/html%c%c", 10, 10); /* write MIME header */
            FCGI_printf("<!-- %s SUPPORTS=FastCGI -->\n", msGetVersion());
            FCGI_fflush(stdout);


            if (msReturnPage(msObj, msObj->Map->web.template, BROWSE, &pszReturn) != MS_SUCCESS)
                writeError();
            else
            {
                FCGI_printf( "%s", pszReturn );
	  ...
	  if (msReturnURL_FastCGI(msObj, msObj->Map->web.template, BROWSE) != MS_SUCCESS)
         writeError();
	}
      }
	...
	break;
      ...
      FCGI_printf("Content-type: %s%c%c",MS_IMAGE_MIME_TYPE(msObj->Map->outputformat), 10,10);
      if( msObj->Mode == MAP )
          status = msSaveImage_FastCGI(msObj->Map,img);
      else
          status = msSaveImage_FastCGI(NULL,img);
	...
	msFreeImage(img);
          ...
          char *pszReturn = NULL;
          if (msReturnTemplateQuery(msObj, "text/html",&pszReturn) 
              != MS_SUCCESS) writeError();
          else
          {
              FCGI_printf("%s", pszReturn);
              msFree( pszReturn );
...
/* -------------------------------------------------------------------- */
    ...
    {
#ifndef _WIN32
        filename = msTmpFile("/tmp/", "tmp" );
#else
        filename = msTmpFile("C:\\", "tmp" );
#endif
    }
...
/* -------------------------------------------------------------------- */
    ...
    if( msSaveImage( map, img, filename ) == MS_FAILURE )
    {
        unlink( filename );
        free( filename );
...
/* -------------------------------------------------------------------- */
/*      Write temporary file through FCGI API.                          */
/* -------------------------------------------------------------------- */
    {
        FILE *fp;
        ...
        while( (bytes_read = fread(block, 1, sizeof(block), fp)) > 0 )
            FCGI_fwrite( block, 1, bytes_read, FCGI_stdout );
