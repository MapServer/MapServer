...
class LayerTestCase(unittest.TestCase):
    def setUp(self):
        self.mapobj1 = mapscript.Map(testMapfile)
    def tearDown(self):
        self.mapobj1 = None
...
# mapscript.Layer ordering tests
...
class ZoomPointTestCase(unittest.TestCase):
    "testing new zoom* methods that we are adapting from the PHP mapscript.MapScript"
...
class RectObjTestCase(unittest.TestCase):
    ...
    def testRectObjConstructorArgs(self):
        r = mapscript.Rect(-1.0, -2.0, 3.0, 4.0)
        assert int(r.minx) == -1
        assert int(r.miny) == -2
        assert int(r.maxx) == 3
        assert int(r.maxy) == 4
    def testRectObjConstructorBadly1(self):
        self.assertRaises(mapscript.MapServerError, mapscript.Rect, 1.0, -2.0, -3.0, 4.0)
    def testRectObjConstructorBadly2(self):
        self.assertRaises(mapscript.MapServerError, mapscript.Rect, -1.0, 2.0, 3.0, -2.0)
...
# mapscript.Point constructor tests
...
class PointObjTestCase(unittest.TestCase):
    ...
    def testPointObjConstructorArgs(self):
        p = mapscript.Point(1.0, 1.0)
        assert int(p.x) == 1
        assert int(p.y) == 1

# mapscript.Color constructor tests
...
class InlineFeatureTestCase(unittest.TestCase):
    ...
    def testAddPointFeature(self):
        ...
        l = mapscript.Line()
        l.add(p)
        shape = mapscript.Shape(inline_layer.type)
        shape.add(l)
